...
*Elasticsearch deep-dive · Part 6 of 8. Previous: [Why results come back ranked: BM25](/elasticsearch-05-relevance-and-bm25/) · Next: [Running at scale: shards, replicas & the write path](/elasticsearch-07-scale-and-the-write-path/).*
...
The fix is the rule from [Part 2](/elasticsearch-02-documents-and-mappings/): **aggregate and sort on `keyword`, numeric, or date fields — never on analyzed `text`.** Those field types build doc_values and store the value whole, exactly one per document, which is what grouping needs.

This is the whole reason dynamic mapping gives a string *two* fields. When Elasticsearch first sees a string like a product name, it maps it as `name` (a `text` field, [analyzed](/elasticsearch-03-text-analysis/) for search) **and** `name.keyword` (a `keyword` field, stored whole for aggregating and sorting). So to group by product name, you write:
