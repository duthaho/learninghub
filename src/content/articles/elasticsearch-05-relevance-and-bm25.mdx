...
*Elasticsearch deep-dive · Part 5 of 8. Previous: [The Query DSL](/elasticsearch-04-query-dsl/) · Next: [Aggregations: the analytics engine](/elasticsearch-06-aggregations/).*
...
- **Analyzers** — synonyms and stemming from [Part 3: Text Analysis](/elasticsearch-03-text-analysis/) decide *what counts as a match* in the first place, which is upstream of every score BM25 computes.

But the pivot from here is bigger than tuning search. In [Part 6: aggregations](/elasticsearch-06-aggregations/), we stop asking "which documents match?" and start asking "what do the matching documents *tell me* in aggregate?" — the move from search engine to analytics engine.
