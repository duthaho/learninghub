...
*Elasticsearch deep-dive · Part 4 of 8. Previous: [How Elasticsearch reads text](/elasticsearch-03-text-analysis/) · Next: [Why results come back ranked: BM25](/elasticsearch-05-relevance-and-bm25/).*
...
Two things make the DSL click, and this part is about both: **which leaf query to use** for a given field, and **which context** that query runs in. The first decides whether your terms even match; the second decides whether the match contributes to ranking. Both build directly on [text analysis](/elasticsearch-03-text-analysis/) — because whether a query analyzes its input is the difference between finding your documents and finding nothing at all.
